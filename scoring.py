...
BLOC3_DIMENSIONS = {
    "qualite_clinique": (1, 3, "Qualité clinique perçue"),
    "utilite_performance": (4, 6, "Utilité perçue / gain de temps (performance)"),
    "ergonomie": (7, 9, "Ergonomie"),
    "integration_flux": (10, 12, "Intégration dans le flux de travail"),
    "fiabilite_securite": (13, 15, "Fiabilité & sécurité perçue des données"),
    "satisfaction_evolution": (16, 18, "Satisfaction globale & intention d'évolution"),
}


def bloc3_dimensions(answers, prefix=""):
    """Détail du Bloc 3 par facette (6 dimensions de 3 items chacune) -> dict {clé: score/100 ou None}."""
    dims = {}
    for key, (start, end, _label) in BLOC3_DIMENSIONS.items():
        total, n = 0, 0
        for i in range(start, end + 1):
            ...
            n += 1
            total += v
        dims[key] = round((total / (n * 5)) * 100) if n else None
    return dims
